...
/*   Updated: 2024/02/24 05:34:40 by lvincent         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_texture_pixel(t_game *game, t_img texture, int lineh, int j)
{
	...
	if (game->raycast.side > 2)
		...
		texX = texture.width - fmod(game->raycast.wall_hit * texture.width / RES, texture.width);
...
void	draw_line(t_game *game, int deg, t_ray ray)
{
	...
	while (++j < 720)
	{
		if (j <= lineo)
			...
			wall_texture(game, ray,deg, j, lineh);
		else
 			mlx_pixel_put(game->mlx, game->win,deg, j,\
